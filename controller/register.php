...
if(!empty($_POST['pseudo_check'])){
    ...
    $q->execute(array($_POST['pseudo_check']));
    $numRows = $q->rowCount();
    if($numRows > 0){
...
if(isset($_POST['pseudo'])){
    ...
    if(empty($pseudo)|| empty($pass1) || empty($pass2) || empty($email)){
        ...
        if(!empty($typecompte) and $typecompte=TYPE_COMPTE_PROFESSIONEL) {
            session_start();
...
//creation de la relation avec sois meme
